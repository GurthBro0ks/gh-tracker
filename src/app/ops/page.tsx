...
function SafetyBadge({ label }: { label: string }) {
  return (
    <span className="rounded border border-amber-300/40 bg-amber-400/10 px-2 py-1 text-[10px] uppercase tracking-[0.14em] text-amber-200">
      {label}
    </span>
  );
}
...
function Card({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="neon-panel rounded-xl p-4">
      <h2 className="font-sans text-sm uppercase tracking-[0.12em] text-fuchsia-200">{title}</h2>
      <div className="mt-3 space-y-3 text-sm text-violet-100/90">{children}</div>
    </section>
  );
}
...
function FixtureList({ lines }: { lines: Array<{ label: string; value: string }> }) {
  return (
    <dl className="space-y-2 text-sm">
      {lines.map((line) => (
        <div key={line.label} className="rounded-lg border border-white/10 bg-black/20 px-3 py-2">
          <dt className="text-[10px] uppercase tracking-[0.12em] text-violet-300/75">{line.label}</dt>
          <dd className="mt-1 text-violet-100">{line.value}</dd>
        </div>
      ))}
    </dl>
  );
...
function PreviewBlock({ lines }: { lines: string[] }) {
  return (
    <div className="rounded-lg border border-cyan-300/20 bg-black/25 p-3 font-mono text-xs text-cyan-100">
      {lines.map((line) => (
...
export default async function HabitatOpsPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-[1400px] px-3 pb-6 sm:px-4 md:px-8" style={{ paddingTop: "max(1rem, env(safe-area-inset-top))" }}>
      <header className="neon-panel mb-4 rounded-xl px-4 py-4 sm:mb-6 sm:px-5 sm:py-5">
      ...
      <section className="mb-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <Card title="Schedule Dry-Run">
          <p>Sample plan target: <span className="font-mono text-lime-300">{fixture.scheduleDryRun.samplePlanTarget}</span></p>
        ...
        <Card title="Workspace Dry-Run">
          <p>
            Canonical preview session: <span className="font-mono text-lime-300">{fixture.workspaceDryRun.canonicalSessionPreview}</span>
          </p>
        ...
        <Card title="Reports">
          ...
          <p>Expected future URL style: <span className="font-mono text-cyan-200">{fixture.reports.expectedUrlPattern}</span></p>
